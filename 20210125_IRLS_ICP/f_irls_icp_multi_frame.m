function [varargout] = f_irls_icp_multi_frame(Refdata, Movdata, Tf0, Kernel, Md)
% refdata -> dim * npt
% movdata -> dim * npt
% tf0 -> (dim + 1) * (dim + 1)
...
for iter = 1:1:maxIter
    ...
    Residuals = [];
    Aftdatas = [];
    for frame_idx = 1:size(Refdata,2)
        RefAlldata = [RefAlldata,Refdata{frame_idx}];
        MovAlldata = [MovAlldata,Refdata{frame_idx}];
        Aftdata = bsxfun(@plus, R * Movdata{frame_idx}, T);
        [ RefIdx, Residual ] = knnsearch( Refdata{frame_idx}', Aftdata' );
        cur_pts = Refdata{frame_idx};
        corres_pts =[corres_pts,cur_pts(:,RefIdx)];
        Residuals = [Residuals;Residual];
        Aftdatas = [Aftdatas,Aftdata];
    ...
    if size(corres_pts,2) ~= 0
        [ dR, dT ] = RegFun(corres_pts, Aftdatas, Weights );
    else
        dR = eye(nDim);
        dT = zeros(nDim,1);
    end
